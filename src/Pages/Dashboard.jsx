...
import {HiChevronDown, HiChevronUp, HiMenuAlt3, HiOutlineLogout} from "react-icons/hi";
...
const Dashboard = () => {
    const menus = [
        ...
        { name: 'Configuration', link: '/Dashboard', icon: IoMdSettings },
        { name: 'Agences', link: '/Dashboard', icon: AiTwotoneShop},
    ...
    const AddPays = () => {
        const inputValue1 = window.prompt('Saisir le Nom du Pays:');
        const inputValue2 = window.prompt("Saisir l'Indicatif");
        const inputValue3 = window.prompt(' Saisir le CodeISO');
        const inputValue4 = window.prompt(' Saisir la Devise du Pays');

        if (inputValue1 && inputValue2 && inputValue3 && inputValue4) {
            console.log('Valeur 1:', inputValue1);
            console.log('Valeur 2:', inputValue2);
        } else {
            console.log('Saisie annulée ou incomplète');
        }
    }
        ...
        const data = [
            {
                ...
                ville: 'Lomé',
                adresse: 'Agoè-nyivé',
                contact: '0022870554812',
                email: 'agence@kora.com'
            },
            {
                id: 2,
                agence: 'Agoenyive2',
                        ...
                        {menus?.map((menu, i) => (
                            <div key={i}>
                                <div
                                    className={`
              ${menu?.margin && 'mt-5'}
              group flex items-center cursor-pointer text-sm gap-3.5 font-medium p-2 hover:bg-gray-800 rounded-md
            `}
                                    onClick={() => {
                                        if (i === 1) {
                                ...
                                >
                                    ...
                                    {i === 1 && (
                                        <div
                                            className='cursor-pointer ml-auto duration-500'
                                            onClick={() => handleAccordionClick(i)}
                                        >
                                            {openAccordion === i ? <HiChevronUp size={20}/> :
                                                <HiChevronDown size={20}/>}
                                        </div>
                                    )}
                                </div>
                                {i === 1 && openAccordion === i && (
                                    <div className='ml-4 transition duration-500'>
                                        <div className='mt-5 ml-5 cursor-pointer' onClick={AddPays}>Ajouter Pays</div>
                                        <div className='mt-5 ml-5 cursor-pointer' onClick=''>Ajouter Ville</div>
                                        <div className='mt-5 ml-5 cursor-pointer' onClick=''>Ajouter Kiosque</div>
                                    </div>
                                )}
                            </div>
                        ))}
                    ...
                    <div className='bg-gray-100 p-4 mt-0 mr-3 shadow-md shadow-gray-600 h-[750px] w-[1240px]'>
                        ...
                        <form>
                            ...
                            <div className='pt-2'>
                                ...
                                <button type='submit'
                                        className='h-10 w-44 shadow-xl rounded-md shadow-green-500 hover:bg-green-900 duration-200 text-white bg-green-700 '>
                                    Enregistrer
                                </button>
                            </div>
                        </form>
                        <div
                            className='bg-gray-200 p-5 mt-10 mr-5 rounded-2xl  shadow-md shadow-gray-600 h-[490px] overflow-y-auto w-[1215px]'>
                                ...
                                {data.map((item) => (
                                    <tr key={item.id}>
                                        ...
                                        <td className='text-center'>
                                            <button type='submit' className='bg-red-700 rounded-sm p-2 text-white'>
                                                <MdDelete/></button>
                                        </td>
                                    </tr>
                                ))}
